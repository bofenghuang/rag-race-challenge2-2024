...
class Reranker:
    ...
    def _predict(self, query, contexts):
        inputs = [
            DEFAULT_TEXT_RERANK_TMPL.format(query=query, context=context)
        ...
        tokens = self._tokenizer(inputs_messages, return_tensors="pt", padding=True).to(
            self._model.device
        )
        ...
        return [
            (
                out["logits"][i][-1][self._no_id] < out["logits"][i][-1][self._yes_id]
            ).item()
    ...
    def __call__(self, query: str, documents: List):
        ...
        retrieved_documents = [
            doc for doc, is_validated_ in zip(documents, is_validated) if is_validated_
        ]
        if len(retrieved_documents) == 0:
            logging.info("No validated documents, retrurning top 2 documents")
            retrieved_documents = sorted(
                documents, key=lambda x: x.embed_score, reverse=True
            )[:2]
        logging.info(
            f"{self.__class__} retrieved {len(retrieved_documents)} documents from {len(documents)}"
        )
