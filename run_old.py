...
from category_retriever import NotHardRetriever
from embed_retriever import BGEM3FlagModelEmbedder, get_top_k_embeddings
...
doc_texts = [n.get_content("all") for n in nodes]

# Setting use_fp16 to True speeds up computation with a slight performance degradation
embed_model = BGEM3FlagModelEmbedder("BAAI/bge-m3", use_fp16=True, device=3)

embeddings = embed_model.embed_documents(doc_texts, max_length=512)
...
result_ids, _ = get_top_k_embeddings(query_embedding, embeddings, score_name, 5)

selected_docs = [doc_texts[i] for i in result_ids]
...
for doc_name_, doc_ in documents.items():
    ...
    index_ = index_.as_retriever(similarity_top_k=5)
    indices[doc_name_] = index_


hybrid_retriever = NotHardRetriever(indices)
